...
const removeFilters = (cols: string[], filters?: any[]) => {
  if (!filters) return [];
  return filters.filter(f => cols.includes(f.col || ''));
};
...
export default function buildQuery(formData: QueryFormData) {
  ...
  const formData2 = {
    ...
    extra_form_data: {
      ...extra_form_data,
      filters: accept_limitted_cross_filters
        ? removeFilters(accept_emit_filters, extra_form_data?.filters)
