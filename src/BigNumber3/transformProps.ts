...
export default function transformProps(chartProps: BigNumberChartProps) {
  ...
  let positiveChangeColor;
  if (positiveColor) {
    const { r, g, b } = positiveColor;
    positiveChangeColor = color.rgb(r, g, b).hex();
  }

  let negativeChangeColor;
  if (negativeColor) {
    const { r, g, b } = negativeColor;
    negativeChangeColor = color.rgb(r, g, b).hex();
  }
